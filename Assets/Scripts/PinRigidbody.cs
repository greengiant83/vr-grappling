...
public class PinRigidbody : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        var angularVelocity = (Time.fixedDeltaTime * angle * axis) * rotMagic;
        if (!float.IsNaN(angularVelocity.x)) rigidbody.angularVelocity = angularVelocity;
    }
}
